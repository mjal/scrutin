...
let make = () => {
  ...
  let (choices, setChoices) = React.useState(_ => ["Placeholder 1", "Placeholder 2"])
  ...
  <>
    ...
    <S.TextInput
      label="" testID="election-title" value=name onChangeText={text => setName(_ => text)}
    />
    <S.Section title={t(. "election.new.question")} />
    <S.TextInput
      label="" testID="election-desc" value=desc onChangeText={text => setDesc(_ => text)}
    />
