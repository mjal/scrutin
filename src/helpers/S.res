...
let title = textStyle(~textAlign=#center, ~fontSize=20.0, ~color=Color.black, ())
...
module Button = {
  @react.component
  let make = (~onPress, ~title, ~style=?, ~titleStyle=?) => {
    ...
    let defaultTitleStyle = textStyle(~fontSize=20.0, ~color=Color.white, ())
...
module TextInput = {
  ...
  let make = (~label=?, ~testID=?, ~value, ~onChangeText) => {
    ...
    <TextInput style mode=#flat ?label ?testID value onChangeText />
  }
}
